...
def load_sweeps():
    """Import all .py files in sweeps/ and collect their OPTIONS dicts."""
    sweeps = {}
...
def run_training(base_config_path, overrides, output_dir, run_name, pbar):
...
def load_sweep_file(path):
    """Load a single sweep .py file and return (name, options)."""
    path = Path(path)
    spec = importlib.util.spec_from_file_location(path.stem, path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return path.stem, mod.OPTIONS
...
def main():
    ...
    if argv and argv[0].endswith(".py") and os.path.isfile(argv[0]):
        sweep_name, options = load_sweep_file(argv[0])
        argv = argv[1:]

    parser = argparse.ArgumentParser(description="Run ECO experiment sweeps")
    parser.add_argument("--base_config", required=True, help="Base TOML config")
    ...
    for variation in pbar:
        ...
        success, elapsed = run_training(
            args.base_config, variation["overrides"], output_dir, run_name, pbar,
        )
