...
- All ECO experiment TOML configs should include the shebang `#!/tmp/mlsweep_shims/run_config.sh`.
  The `start_aim_server.sh` script generates wrapper scripts in `/tmp/mlsweep_shims/` that
...
- Run `./start_aim_server.sh` once (or just its wrapper-generation portion) to
  create `/tmp/mlsweep_shims/run_config.sh` and `/tmp/mlsweep_shims/run_sweep.py` before executing
...
The `/tmp/mlsweep_shims/run_sweep.py` wrapper (generated by `start_aim_server.sh`) reads the
